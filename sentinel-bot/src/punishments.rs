...
use poise::serenity_prelude::{
  ...
  CreateEmbedFooter, CreateMessage, CreateTextDisplay, MessageFlags, User,
};
...
impl PunishmentDisplay<'_> {
  ...
  pub fn from_punishment_type<'a>(kind: &PunishmentType) -> PunishmentDisplay<'a> {
    match kind {
      PunishmentType::KICK => PunishmentDisplay::from(
        "<:dot_gray:1499540057172742174> kick",
        0xB3968D,
        "You have been kicked.",
      ),
      PunishmentType::WARN => PunishmentDisplay::from(
        "<:dot_orange:1499540055402745876> warn",
        0xFAA620,
        "You have been warned.",
      ),
      PunishmentType::BAN => {
        PunishmentDisplay::from("<:dot_red:1499540053737865457> ban", 0xEF4949, "You have been banned.")
      }
      PunishmentType::TIMEOUT => PunishmentDisplay::from(
        "<:dot_purple:1499540059672805546> timeout",
        0x9A59B4,
...
pub async fn execute_ban(
  ...
  let delete_seconds = if delete_messages {
  ...
  ctx
    .http()
    .ban_user(
      ctx.guild_id().expect("Context outside of guild"),
      member.id,
      delete_seconds as u32,
      reason.as_deref(),
    )
    .await?;
  Ok(())
...
fn get_embeds<'a>(
  ...
  guild_icon: String,
) -> Result<(CreateComponent<'a>, Option<CreateMessage<'a>>), Error> {
  ...
  if let Ok(duration) = punishment.duration.clone().ok_or_else(|| "Not present") {
    components.push(format!(
      "**Duration**: ~{} (expiry: <t:{}:F>",
      duration.display,
      duration.to_unix_time_from_now()
    ));
  }

  let components: Vec<CreateContainerComponent> = components
    .into_iter()
  ...
  if punishment.punishment_type == PunishmentType::NOTE {
    ...
    let msg = CreateMessage::new().embed(
      CreateEmbed::new()
        ...
        .field(
          "Reason",
          punishment.reason.clone().unwrap_or(String::from("(not specified)")),
          true,
        )
        .footer(CreateEmbedFooter::new(format!(
          "Punishment: {}",
          punishment.punishment_id.as_simple()
        ))),
    );
